 ...
 *
 * 1. DropzoneTransfer drop — snippet dragged from the Dropzone sidebar. a. Clone-aware : file is part of a loaded clone group → apply the pre-computed "Extract Method" across all clone sites. b. Generic wrap: no clone context → prompt for a method name and insert the snippet wrapped in a language-appropriate function.
 *
 * 2. Plain TextTransfer drop — ordinary editor-to-editor text drag (plain copy/move with no refactoring intent).
 *
...
public class EditorDropStartup implements IStartup {
   ...
   /**
    * Loads all_refactor_results.json from the runtime workspace into {@link CloneContext} so that drag-drop can find clone records even before the user opens the Clone Tree view.
    *
    * Uses {@code Platform.getLocation()} (the Eclipse runtime workspace root) instead of a hardcoded path so it works on any machine.
    */
   private void tryAutoLoadJson() {
      try {
         org.eclipse.core.runtime.IPath wsLoc = org.eclipse.core.runtime.Platform.getLocation();
         if (wsLoc == null) {
            return;
         }
         ...
         String[] candidates = {
               ...
               base + rr + "/systems/all_refactor_results.json", base + "/systems/all_refactor_results.json", base + rr + "/all_refactor_results.json", base + "/all_refactor_results.json", };
         for (String jsonPath : candidates) {
            ...
            try (FileReader reader = new FileReader(f)) {
               Gson gson = new Gson();
               Type type = new TypeToken<List<CloneRecord>>() {
               }.getType();
               ...
               if (records != null && !records.isEmpty()) {
                  ...
                  System.out.println("[refactor_plugin] auto-loaded " + records.size() + " records from " + jsonPath + " (workspaceRoot=" + ctx.workspaceRoot + ")");
   ...
   private void attachDropListener(ITextEditor editor) {
      ...
      if (Boolean.TRUE.equals(widget.getData("refactor.dnd.attached"))) {
         return;
      }
   ...
   private void handleDropzoneSnippet(ITextEditor editor, String snippet, Shell shell) {
      ...
      if (record != null) {
         ...
         // ── Generic wrap path ─────────────────────────────────────────
         InputDialog dlg = new InputDialog(shell, "Wrap in Extracted Method", "Name for the method that will wrap the dropped snippet:", "extractedMethod", v -> v.matches("[a-zA-Z_$][\\w$]*") ? null : "Please enter a valid Java identifier.");
   ...
   /**
    * Attaches an {@link IDocumentListener} that mirrors the VS Code {@code onDidChangeTextDocument} drag-listener logic.
    *
    * VS Code fires one event with TWO content-changes (delete + insert of the same text) when the user drags a block within the same file. Eclipse fires two consecutive {@code documentChanged} callbacks instead.
    *
   ...
   private void attachDragDetector(ITextEditor editor, StyledText widget) {
      ...
      doc.addDocumentListener(new IDocumentListener() {
         ...
         public void documentChanged(DocumentEvent e) {
            ...
            int gapEnd = insOff;
            if (delOff < 0 || gapStart < delOff || gapEnd > postDrag.length()) {
               return;
            }
            String preDrag = postDrag.substring(0, delOff) + body + postDrag.substring(delOff, gapStart) + postDrag.substring(gapEnd);

            final CloneRecord rec = record;
            ignoreUntil[0] = now + 3000; // suppress further events while dialog is open

            Display.getDefault().asyncExec(() -> {
               Shell shell = widget.isDisposed() ? Display.getDefault().getActiveShell() : widget.getShell();

               boolean ok = MessageDialog.openConfirm(shell, "Apply Extract Method", "Apply \"Extract Method\" for clone group \"" + rec.classid + "\"?\n\n" + rec.sources.size() + " clone site(s) will be updated.\nUse Ctrl+Z to undo.");
               ...
               try {
                  ...
                  MessageDialog.openError(shell, "Revert Error", "Could not revert drag: " + ex.getMessage());
                  ignoreUntil[0] = 0;
                  return;
               }
               ...
               MessageDialog.openInformation(shell, "Extract Method Applied", "Done: " + rec.classid + " \u2014 " + rec.sources.size() + " site(s) updated.");
            });
         }
      });
   }
   ...
   /**
    * Finds the clone record for {@code filePath} using three strategies in order of specificity:
    *
    * <ol>
    * <li>Exact match via {@code lastOpenedByFile} (fastest — set when the user double-clicks a source node in the Clone Tree).</li>
    * <li>Exact resolved-path match using {@code CloneContext.resolvePath()} (works when {@code workspaceRoot} is set correctly).</li>
    * <li>Path-suffix match — the absolute OS path always ends with the relative JSON source path, regardless of whether {@code workspaceRoot} is set or what path prefix Eclipse reports. This is the most robust fallback and mirrors how VS Code's {@code document.uri.fsPath} always gives an absolute path that contains the relative JSON path as a suffix.</li>
    * </ol>
    */
    ...
    *
    * 1. {@code ILocationProvider} adapter — implemented by {@code FileEditorInput} (Package Explorer files). Returns the absolute OS path via {@code IFile.getLocation()}. This is the primary path for files opened from the Package Explorer and was the missing link: {@code IFileEditorInput.getAdapter(IPath.class)} returns {@code null}, so the previous IPath-first logic always fell through to {@code null} for those files.
    *
    * 2. {@code IPath} adapter — works for {@code IFileStoreEditorInput} (EFS / external files, e.g. opened by CloneTreeView). Uses {@code makeRelative()} before {@code IPath.append()} to prevent the silent "absolute IPath passed to append() returns itself" bug.
    *
   ...
   private String getEditorFilePath(ITextEditor editor) {
      ...
      try {
         ...
         /* not an IFileEditorInput — try next strategy */ }
   ...
   private String detectLanguage(ITextEditor editor) {
      var ipath = editor.getEditorInput().getAdapter(org.eclipse.core.runtime.IPath.class);
      if (ipath == null) {
         return "java";
      }
   ...
   private String getCaretIndent(ITextEditor editor) {
      ...
      try {
         ...
         String lineText = doc.get(doc.getLineOffset(lineIdx), doc.getLineLength(lineIdx));
   ...
   private void insertAtCaret(ITextEditor editor, String text) {
      ...
      try {
         doc.replace(ts.getOffset(), 0, text);
      } catch (Exception e) {
