...
public class DropzoneView extends ViewPart {
   ...
   public static class DropItem {
      ...
      public final String enclosingFunctionName;
      public final String snippetPreview;

      public DropItem(String content, CloneDragPayload payload, int startLine, int length, String className, String enclosingFunctionName, String snippetPreview) {
         this.content = content;
         this.payload = payload;
   ...
   @Override
   public void createPartControl(Composite parent) {
      tableViewer = new TableViewer(parent, SWT.BORDER | SWT.FULL_SELECTION | SWT.V_SCROLL | SWT.H_SCROLL | SWT.MULTI);
   ...
   private void createColumns() {
      createColumn("Start", 70, item -> Integer.toString(item.startLine));
      createColumn("Length", 70, item -> Integer.toString(item.length));
      createColumn("Class", 140, item -> item.className);
      createColumn("Enclosing Function", 180, item -> item.enclosingFunctionName);
      createColumn("Snippet", 420, item -> item.snippetPreview);
   }

   private void createColumn(String title, int width, Function<DropItem, String> valueProvider) {
      ...
      col.setLabelProvider(new ColumnLabelProvider() {
         ...
         public String getText(Object element) {
            return valueProvider.apply((DropItem) element);
         }
      });
   }
   ...
   private void contributeToolbar() {
      ...
      removeAction.setToolTipText("Remove the selected row(s) from the Dropzone");

      Action clearAction = new Action("Clear All") {
         @Override
         public void run() {
            if (!items.isEmpty() && MessageDialog.openConfirm(getSite().getShell(), "Clear Dropzone", "Remove all rows?")) {
               items.clear();
               tableViewer.refresh();
            }
         }
      };
      clearAction.setToolTipText("Clear all rows from the Dropzone");
   ...
   private void addFromEditorSelection() {
      ...
      String className = inferClassName(selectedSource);
      String functionName = inferEnclosingFunctionName(selectedSource);
      String preview = buildSnippetPreview(ts.getText());

      DropItem item = new DropItem(ts.getText(), payload, startLine, length, className, functionName, preview);
      addSnippet(item);
   }
   ...
   private String buildSnippetPreview(String text) {
      if (text == null)
         return "";
   ...
   private String inferClassName(CloneRecord.CloneSource src) {
      if (src == null || src.file == null)
         return "";
   ...
   private String inferEnclosingFunctionName(CloneRecord.CloneSource src) {
      if (src == null || src.enclosing_function == null || src.enclosing_function.qualified_name == null) {
         return "";
      }
      String qn = src.enclosing_function.qualified_name.trim();
      int idx = qn.lastIndexOf('.');
      return idx >= 0 ? qn.substring(idx + 1) : qn;
   }

   public void addSnippet(DropItem item) {
